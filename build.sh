...
# Cloud front tests need blip to be build
npm run test-cloudfront

# Move blip build to it's final destination
mv -v packages/blip/dist server/dist
...
if [ "${ARTIFACT_NODE_VERSION}" = "${TRAVIS_NODE_VERSION:-0.0.0}" ]; then
  ...
  publishDockerImage
  npm install --save-dev "ci-toolbox@2.4.1"
  BUILD_SOUP="true"
  buildSOUP
else
