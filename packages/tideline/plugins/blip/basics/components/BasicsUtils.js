...
export function labelGenerator(opts) {
  ...
  return {
    bg: {
      verylow: t('below') + ' ' + format.tooltipBGValue(bgClasses['very-low'], opts.bgUnits) + bgUnits,
      low: t('between') + ' ' + format.tooltipBGValue(bgClasses['very-low'], opts.bgUnits) + ' - ' + format.tooltipBGValue(bgClasses.low, opts.bgUnits) + bgUnits,
