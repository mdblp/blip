...
describe('Signup stepper', () => {
  ...
  function checkAccountSelectorStep(role: UserRoles) {
    ...
    if (role === UserRoles.hcp) {
      act(() => {
        fireEvent.click(hcpRadioInput)
      })
    }
  ...
  function checkConsentStep(role: UserRoles) {
    ...
    if (role === UserRoles.hcp) {
      ...
      expect(feedbackCheckbox).not.toBeInTheDocument()
    }

    fireEvent.click(nextButton)
  }

  async function checkProfileStep(role: UserRoles) {
    ...
    if (role === UserRoles.hcp) {
      ...
      expect(hcpProfessionSelector).not.toBeInTheDocument()
    }
  ...
  it('should be able to create a caregiver account', async () => {
    await renderDom()
    checkAccountSelectorStep(UserRoles.caregiver)
    checkConsentStep(UserRoles.caregiver)
    await checkProfileStep(UserRoles.caregiver)
  })

  it('should be able to create a hcp account', async () => {
    await renderDom()
    checkAccountSelectorStep(UserRoles.hcp)
    checkConsentStep(UserRoles.hcp)
    await checkProfileStep(UserRoles.hcp)
  })
})
